...
class mpc_config:
    ...
    # Gain Matrices
    Rk: list = field(
        #default_factory=lambda: np.diag([0.001, 0.001])
        default_factory=lambda: np.diag([0.001, 1.0])
    ...
    Qk: list = field(
        #default_factory=lambda: np.diag([3.0, 3.0, 2.0, 4.0]) # smooth
        default_factory=lambda: np.diag([13.5, 13.5, 12.0, 0.001]) # swervy
        #default_factory=lambda: np.diag([13.5, 13.5, 2.0, 10.0])
    )  # state error cost matrix, for the the next (T) prediction time steps [x, y, v, yaw]
    Qfk: list = field(
        #default_factory=lambda: np.diag([14.5, 14.5, 3.0, 10.0]) #smooth
        default_factory=lambda: np.diag([14.5, 14.5, 13.0, 0.1]) #swervy
        #default_factory=lambda: np.diag([3.0, 3.0, 2.0, 5.0])
...
class MPC(Node):
    ...
    def __init__(self):
        ...
        # get waypoints here
        csv_file_path = './src/f1tenth_gym_ros/rcws/logs/wp-2023-11-15-22-37-02.csv' # slam map
        waypoint_list = []
        with open(csv_file_path, 'r') as csvfile:
            reader = csv.reader(csvfile)
            for row in reader:
                if len(row) >= 4:
                    waypoint_list.append([float(row[0]), float(row[1]),float(row[2]),float(row[3])])
        ...
        waypoints_path_msg = Path()
        waypoints_path_msg.header.frame_id = 'map'
        for row in self.waypoints:
            ...
            waypoints_path_msg.poses.append(pose)
    ...
    def getyaw_from_quaternion(self, quaternion):
        """
    ...
    def pose_callback(self, pose_msg):
        ...
        vehicle_state.yaw = self.getyaw_from_quaternion(pose_msg.pose.pose.orientation)
    ...
    def mpc_prob_init(self):
        ...
        final_state_deviation = self.xk[:, -1] - self.ref_traj_k[:, -1]
        objective += cvxpy.quad_form(final_state_deviation, self.config.Qfk)
